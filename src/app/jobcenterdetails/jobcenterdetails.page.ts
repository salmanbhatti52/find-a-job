...
@Component({
  selector: 'app-jobcenterdetails',
  templateUrl: './jobcenterdetails.page.html',
  styleUrls: ['./jobcenterdetails.page.scss'],
})
export class JobcenterdetailsPage implements OnInit {
  ...
  ionViewWillEnter() {
    if (this.requestsType) {
      ...
      if (this.requestsType === 'appointment') {
        this.mySegment.nativeElement.children[1].click();
        this.jobssection = true;
      }
    } else {
  ...
  segmentChanged(ev) {
    ...
    if (ev.detail.value === 'appointment') {
      this.requestsType = 'appointment';
      this.navCtrl.navigateForward('jobappointment');
    }
